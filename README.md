...
| **MCP servers** declared in `.mcp.json` — `gitmcp` (third-party API docs; which server plays that role is named in `.claude/docs-provider.json`, so swapping it is a config edit), `plantuml` (diagram render), `playwright` (cross-engine smoke), `baseline` (coordination channel) | 4 | `.mcp.json` |
