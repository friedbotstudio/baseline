...
The Claude Code Baseline is a repository overlay shipped via `npx @friedbotstudio/create-baseline ./target`. It installs **26 hooks** at Claude's tool boundaries, **52 skills** organised into fifteen categories, **1 subagent** for parallel work in isolated worktrees, **9 canonical workflow tracks** declared in `.claude/workflows.jsonl` (where `intake-full` runs the full 11-phase pipeline from intake to commit), and **3 user-typed consent gates** that Claude cannot forge.
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------: | ---------------------------------------------------- |
| **Hooks** at PreToolUse / PostToolUse / SessionStart / Stop / PreCompact / UserPromptSubmit / Notification                                                                                                                                                                                                                                                 |                   26 | `.claude/hooks/`                                     |
...
| **Workflow tracks** declared in `.claude/workflows.jsonl`. Canonical set: `intake-full` (the full 11-phase pipeline), `spec-entry`, `tdd-quickfix`, `chore`, `freeform` (ad-hoc heterogeneous batch; all hooks remain active), `epic` (discovery once, sliced spec), `epic-child` (inherits epic discovery), `org` (peer sessions; opt-in, off by default), `power` (batch sprint; opt-in, off by default). Two sub-tracks (`swarm-implementation`, `tdd-worker-chain`) are referenced by selector nodes inside the canonical set. | 9 selectable + 2 sub | `.claude/workflows.jsonl`, enforced by `track_guard` |
| **Consent gates** — `/approve-direction`, `/approve-swarm`, `/grant-commit` are the three workflow-phase gates; `/grant-push` is a fourth, runtime gate for protected-branch pushes. All user-typed; structurally un-invokable by Claude                                                                                                                         |    3 phase + 1 runtime | `consent_gate_grant` UserPromptSubmit hook           |
| **MCP servers** declared in `.mcp.json` — `context7` (third-party API docs), `plantuml` (diagram render), `playwright` (cross-engine smoke), `sprint-channel` (sprint-mode coordination channel)                                                                                                                                                                                                                |                    4 | `.mcp.json`                                          |

The roster counts are asserted by `audit-baseline` against `docs/init/seed.md` and the manifest on every build, and drift fails CI. The cross-doc scanner reads prose claims, not table cells, so this table is maintained by hand against the same source of truth.
...
# Skip the CI/secrets posture (gitleaks pre-commit gate, scripts/ci helpers, branch-protection config)
npx @friedbotstudio/create-baseline ./your-project --no-ci-posture
```

By default the scaffolder writes inside `.claude/`, plus `CLAUDE.md`, `.mcp.json`, `docs/init/seed.md`, and a small CI posture set: `.githooks/pre-commit` (a gitleaks secrets gate), three `scripts/ci/` helpers, and a fill-in branch-protection config at `.github/branch-protection/main.json`. If your project already runs its own secrets scanning or branch protection, pass `--no-ci-posture`: the install skips those files and sets `ci_posture.enabled: false` in `project.json`, and later upgrades will not re-deliver them or touch your own hooks. Pass `--with-npmrc` to also drop `ignore-scripts=true` + `min-release-age=7` into `target/.npmrc`. Those defaults blunt the npm post-install-hook attack class and delay consumption of fresh malicious publishes. An existing `target/.npmrc` is preserved verbatim. Operators who already set these defaults in `~/.npmrc` don't need the flag.
...
- **`/approve-direction <slug>`** — early, at intake (gate A): authorizes the build direction before scout/research/spec; the spec is then machine-reviewed, not human-gated
...
4. A passing `node .claude/skills/audit-baseline/audit.mjs` (which checks all five for drift; the MCP check asserts the servers by name rather than by count)
