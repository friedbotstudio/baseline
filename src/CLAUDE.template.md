...
**Enforcement.** The 26 hooks in `.claude/hooks/` are the structural enforcement layer of this constitution. Each is mapped to the Article it enforces in **Article VIII**.
...
4. **Memory check.** The `memory_session_start` hook injects a memory index into your additional context — per-file entry counts, stale rows, the architecture-map concept list, and the resume snapshot. The index **reports** the `_pending.md` candidate count in its table and prompts no action on it: **Phase 10.7** flushes inside every workflow, so a session-start nag announced work the pipeline already performs. Curate early with `/memory-sync` when you want to; nothing asks you to.
...
| 10.7 | Memory flush | `/memory-sync` | curated canonical memory + reset `_pending.md` |
...
- Freeform → `/triage` selects the `freeform` track for ad-hoc **heterogeneous** batches. Every pre-commit phase is a blanket exception; the DAG carries only `roadmap-sync` → `memory-sync` → `/grant-commit` → `/commit`. All 26 hooks stay active, including `tdd_order_guard` and the consent gates. Anything single-purpose with a clear failing-test path SHALL route to `tdd` or higher. Full exception list: annex §5.13.
...
**Swarm vs solo at Phase 6.** Swarm dispatch is the **default** code-generation route: every code-generating track carries an `implementation` selector whose swarm alternate is taken when the approved spec has ≥ `project.json → swarm.min_tasks_worth_swarming` (default 1) independent components **and** the project is a git repository; otherwise the solo `/tdd` chain runs. On a non-git tree the swarm phases are excepted at triage time, so this always resolves to solo, and a user "use swarm" override SHALL be refused with the reason `swarm requires git`.
...
The 26 hooks in `.claude/hooks/` are the structural enforcement of this constitution. Modifying, disabling, or bypassing a hook requires explicit user approval and a `seed.md` §4.1 amendment. The table names each hook, its event, and the Article it enforces; fuller per-hook behavior lives in `.claude/CONSTITUTION.md` (annex).
...
3. Treat `_pending.md` as the auto-extraction inbox. Promote candidates to canonical files only via `/memory-sync`; you SHALL NOT write canonical files directly. Mechanics + the phase-skill carve-out: annex.
4. Treat `_resume.md` as the cross-session continuity snapshot. It is **session memory**, not project memory; refresh cadence: annex.
5. Respect `size-cap: 500` per canonical file **in the flat shape**. Entries unverified for ≥ 30 commits or ≥ 30 days are stale. Overflow, shard-shape and decay handling: annex.
6. **Preserve verbatim.** Memory entries with `source: user-instruction` or `source: user-feedback` SHALL include a `verbatim:` blockquote of the user's actual words. The verbatim is canonical; the entry body is Claude's interpretation. When verbatim and interpretation conflict, **verbatim wins**, and you SHALL surface the conflict to the user before acting on the interpretation. `/memory-sync` SHALL reject promotions to canonical files that lack a required verbatim. Schema: `.claude/memory/README.md → Source provenance`.
...
8. **Durable local thread trail.** `.claude/memory/_thread.md` is a third memory class — **local + durable**: gitignored content, and OUTSIDE `/memory-sync`'s reset path, so a shelved thread survives flushes and `/clear`. Claude Code (never the human) shelves and resumes it. Detail: `.claude/CONSTITUTION.md` (annex).

9. **The central system spec is not memory.** The structural model lives at `docs/system/`: a reviewed spec artifact the canonical list never walks. Gated by `memory.architecture_map.enabled`. Genesis: `seed.md §4.8`.
