...
- **Current docs for every library API.** API shape confirmed against current docs (the `context7` MCP is the default; official docs / `llms.txt` / a pinned local cache also satisfy it), not training recall. Record the library version.
