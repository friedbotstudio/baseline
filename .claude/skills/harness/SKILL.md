...
**Gate-A content re-check on resume (spec-content-hash).** When the satisfied gate is `approve-spec`, before marking it `completed` the harness recomputes the spec's content hash and compares it to the token. Read line 5 (the content hash) of `.claude/state/spec_approvals/<slug>.approval`; recompute `computeSpecContentHash` of the current `docs/specs/<slug>.md` bytes via `.claude/hooks/lib/spec-content-hash.mjs`; compare with `compareSpecHash(tokenHash, specBytes)`. On **match**, proceed as above. On **mismatch** — the spec was amended after approval — **re-yield at gate A**: remove `approve-spec` from `workflow.json → completed`, set the gate task back to pending, and write `harness_state: yielded` (`reason: "spec amended after approval; re-approve"`). This mechanizes the manual revoke discipline: an untracked (first-time) spec whose git SHA is `N/A` still carries a content hash, so a post-approval edit is now detected structurally rather than by hand. Fail-safe: an absent or `N/A` content hash (a token predating this feature) compares false and re-yields, so a stale token never silently satisfies the gate.
