...
- Path: `.claude/skills/memory-index/lift-fields.mjs` (now the ONLY definition) ← formerly duplicated at `.claude/skills/memory-index/migrate.mjs:81` and `.claude/skills/memory-flush/shape.mjs:40`.
- Trap: the T4 shard migration's field-lifting regex existed in TWO byte-identical copies. The repair spec named only `migrate.mjs`, because that is where the bug was *observed*. The second copy sat on a **write path**: `sweep.mjs:88` round-trips shards through `shape.mjs → writeShardedFromFlat` on every `stamp-closure` (fired automatically by `/commit`) and `auto-close` (fired by `/memory-flush`).
