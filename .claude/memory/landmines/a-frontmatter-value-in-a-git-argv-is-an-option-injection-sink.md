...
- **The fix that holds.** Validate before interpolating: a stamp is `/^[0-9a-f]{7,40}$/` and nothing else, and a rejected one is treated exactly like an unresolvable one. `usableStamp` in `.claude/hooks/lib/staleness.mjs` is the shared predicate. Since `session-start-stale-cache` (2026-08-26) the callers no longer gate individually: every changed-set request goes through `memory_changed_set.mjs -> changedSince`, which calls `usableStamp` before it builds an argv, so there is one gate rather than a copy per reader. Cache keys are stamps read back off disk and are never iterated into a git command.
