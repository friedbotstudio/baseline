...
- **This defeats the sibling landmine's own check.** [[discard-ledger-is-inert-until-memory-sync-step-4-5-runs]] says to verify with `ls .claude/memory/_discard-ledger.md`, because an absent file means the step never ran. Here the file IS created. Presence proves the step ran; it does not prove the rows match anything. Verify the SHAPE of the rows, or re-run capture and confirm the candidate does not return.
...
- Companions: `.claude/skills/memory-sync/ledger.mjs:74`, `tests/ledger-key-form.test.mjs` (the coupling test `test_when_memory_stop_builds_keys_then_every_key_satisfies_is_candidate_key` is what fails if the two definitions ever drift apart).
