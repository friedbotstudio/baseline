...
why: the enforcement layer is tool-aware. An SOP that says "write the token" without binding the tool invites a Bash redirect that is structurally guaranteed to be blocked — observed: `/approve-spec` (since renamed `/approve-direction`) tried a Bash heredoc → destructive-guard block, then `command not found: dirname` under a stripped PATH. Binding the tool removes the detour.
...
- how to apply: the 4 gate commands (approve-direction, approve-swarm, grant-commit, grant-push) and the harness/integrate/tdd/verify SKILLs cite §2 directly and name the tier. New SOPs that write state cite §2. Never grant `Bash(tee:*)` in a gate command's `allowed-tools` (it's a consent-path write-verb the destructive guard blocks).
