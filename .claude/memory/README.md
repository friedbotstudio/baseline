...
Persistent project knowledge that travels with the repo. Loaded into Claude's context at session start (via the `memory_session_start.mjs` hook) and updated as a byproduct of phase skills doing their normal work, plus auto-extracted candidates from the `memory_stop.mjs` hook (curated via `/memory-sync`).
...
| `backlog.md` | `/memory-sync` | Future-work intent captured automatically by `memory_stop.mjs` (intent-line extraction from user prompts and assistant text). Stale-exempt. |
...
| `_pending.md` | `memory_stop.mjs` (writes), `/memory-sync` (clears) | Auto-extracted candidates awaiting curation. **Content gitignored**; the file structure is committed. |
...
`/memory-sync` SHALL reject any candidate promotion to a canonical file when `source` is `user-instruction`, `user-feedback`, or `assistant-deferral` and `verbatim:` is missing or empty.
...
**Migration is lossless and reversible.** `node .claude/skills/memory-index/migrate.mjs --forward|--reverse --root .claude/memory` explodes the flat files into per-fact dirs (proving file-count == block-count before removing any source) and back. The **filename** is a CWE-22-safe slug of the heading; the **key** is the heading verbatim, so `path:line` / `lib@version` stable keys survive. The session-start index is built from frontmatter only, by `buildIndex` in `.claude/hooks/lib/memory_session_start.mjs`, which keeps the upfront context cheap.

**Two functions are named `buildIndex`.** The session-start one is the hook lib named above. The other lives in `.claude/skills/memory-index/build-index.mjs` and assembles the `links:` graph; the SessionStart hook does not import it. `links` was dropped from `LIFTABLE_FIELDS` because no entry in this corpus carries it, so that graph has neither data nor a production caller. Anyone sent there looking for the session-start payload is in the wrong module.
...
**Sweeps are gated on a repaired store.** `sweep.mjs` refuses *every* mode while any allowlisted bullet is still stranded — including `stamp-closure` (fired automatically by `/commit`) and `auto-close` (fired by `/memory-sync`). The remedy is the `--relift` command above, and the error names it. The guard exists because an unrepaired store makes its two readers disagree: the session-start index reads frontmatter only, while `sweep.mjs` reads frontmatter *and* body, so they report different stale counts. Curating against the larger set while every other surface believes the smaller one churns entries without fixing the invisibility.
...
Two equivalent closure forms cause `/memory-sync` Step 0a to delete the entry block on its next run. **Both are first-class** — `/memory-sync sweep --mode auto-close` accepts either.
...
**Per-file invariant**: on `pending-questions.md`, `superseded-at:` MUST NOT appear; on the other five canonical files, `resolved-at:` MUST NOT appear. Mutually exclusive at the file level. Not enforced by audit — documented invariant only. The `/memory-sync` Step 0a sweep flags violations in its report rather than deleting.
...
A match without a corresponding structured closure field or heading suffix causes `/memory-sync` Step 0b to surface a once-per-entry `Close <key> from <file>? (y / n / skip)` prompt.
...
**Automated closure-stamp on backlog pickup.** When `/triage` records a workflow that picks up a backlog entry (the `workflow.json → source_backlog_keys` array carries the entry's stable key), `/commit` Step 6 invokes `node .claude/skills/memory-sync/sweep.mjs --mode stamp-closure --memory-dir .claude/memory --backlog-keys <csv>` after `git commit` succeeds. The mode writes `status: picked-up` + `superseded-at: <today>` to each named entry; the next `/memory-sync` Step 0a auto-deletes them. `/commit` is the only caller of this mode; `sweep.mjs` is the only writer to `backlog.md` during closure-stamping — the curator-not-writer pattern is preserved through the actuator boundary.
...
2. **Stop hook auto-extraction.** `memory_stop.mjs` reads the just-completed turn's transcript, extracts touched paths / cited library APIs / verbalized decisions, and appends candidates to `_pending.md`. Claude reviews via `/memory-sync` and commits keepers to canonical files.
...
`_thread.md` is a **third class — local + durable**. Unlike the canonical categories it is gitignored and never committed (continuity is per-developer noise across a team); unlike `_resume.md` it is append-only and durable rather than overwritten each turn. `/memory-sync`'s reset explicitly excludes it, so a shelved thread survives a flush or `/clear`. It is the durable home for the "what we were working on, and why" narrative that `_resume.md` only ever holds for the latest turn.
...
- `- route: unassigned` — the routing bucket. `unassigned` at capture; a human (with an optional model suggestion from `.claude/skills/memory-sync/route.mjs`) assigns the final bucket at `/memory-sync`. Promotion to canonical stays human-only (Article IX.3); the route is only ever a suggestion.
