...
- progress (2026-06-03): Designed + partially shipped via the `llm-assisted-memory-capture-routing` workflow. **Tier 1 shipped** (DP3 — shared `NOISE_PREFIXES`/`isBoilerplate` in `.claude/hooks/lib/common.mjs`; capture-time boilerplate filter in `shelve_capture.extract`; `memory_stop`/`resume_writer` converged onto the shared source). **Design = Option C (hybrid)** recorded in the approved+archived spec `docs/archive/2026-06-03/llm-assisted-memory-capture-routing/spec.md` (also intake/scout/research in that bundle). **Deferred to follow-up workflows:** Tier 2 (sentence-granularity capture + small expanded cue set + `_pending` route/weight fields + fixture corpus; ABSORBS 91a3 Tier-2 semantic weighting) and Tier 3 (flush-time routing/weighting reading transcript+`_pending` at Sonnet-tier, accept/override default; durable pinned working thread in `_thread.md`; `_resume` rework). Key corrected premise from the build: `memory_stop.iterIntentMatches` is line-SPLIT and matches triggers ANYWHERE in a line (not line-anchored as originally written) — the real gaps are trigger-vocabulary coverage + capture granularity, not anchoring. Resolved gate-A decisions: recall floor 80% mid-sentence, 0 known-boilerplate noise; one trail with pinned working thread; accept/override flush UX; Sonnet-tier flush routing. Pick up the follow-ups from the archived spec.
...
## a-sound-early-short-circuit-is-possible-if-6f65

> verbatim (assistant-deferral, 2026-06-03):
> A *sound* early short-circuit is possible — `if (!/consent|approval|grant/i.test(scan)) return false;` before `resolveAssignments` (every consent basename contains one of those substrings, including in a var value, so it's a safe necessary-condition prefilter).

- source: assistant-deferral
- status: open
- raised-on: 2026-06-03
- raised-in-context: the `guard-postexpand-check-comment` workflow (commit 698fd1a), reviewing `writesConsentPath` in `.claude/hooks/lib/common.mjs`. The consent-path scan runs `expandWithEnv(scan, resolveAssignments(scan))` (`.claude/hooks/lib/common.mjs:693`) on every Bash command before checking whether the resolved target is a consent path. `resolveAssignments` (`:647`) is the more expensive step; the vast majority of Bash commands touch no consent path at all.
- estimated-effort: small
- verified-at: 698fd1a
- last-touched: 2026-06-03
- caveat: Necessary-condition prefilter: every consent marker/token basename (`commit_consent`, `push_consent`, `.commit_consent_grant`, `spec_approvals`, `swarm_approvals`, `.*_grant`) contains one of `consent` / `approval` / `grant` as a substring, so a command whose full text matches none of those (case-insensitive) cannot possibly write a consent path — returning `false` before `resolveAssignments` is sound. Watch-items when picked up: (1) confirm the substring set stays exhaustive if new consent path names are added later (couple the regex to the same source the path-detection uses, or add a test that every consent basename matches the prefilter); (2) the prefilter must run on the RAW command text (`scan`) BEFORE assignment resolution, but a consent path produced only via an env-var value (`X=.../commit_consent; ... $X`) still contains the substring in the assignment, so the raw-text test remains sound — add a test for that case. Micro-optimization only; no behavior change intended. Cross-ref: `.claude/hooks/lib/common.mjs:690` (`writesConsentPath`), `:647` (`resolveAssignments`), `:693` (call site).

## drift-check-scores-spec-doc-in-working-tree-trivial-pass-a1b2

> verbatim (assistant-deferral, 2026-06-03):
> drift_check now reads the working tree (good), but the spec markdown itself is in that diff, so every AC-NNN string self-satisfies and all ACs report resolved trivially — a false pass

- source: assistant-deferral
- status: open
- raised-on: 2026-06-03
- raised-in-context: the `llm-assisted-memory-capture-routing` workflow's drift-check-tick. After backlog `d1f7` fixed `loadDiff` to source the working tree, drift_check exited 0 with ALL 14 ACs "resolved" — but the evidence showed each AC matched a `+`-line that was the spec markdown's own `| AC-NNN | ... |` row (the untracked `docs/specs/<slug>.md` is part of the working-tree diff). So the working-tree fix introduced a NEW trivial-pass: the spec document scoring against itself.
- estimated-effort: small
- verified-at: 12db8fc
- last-touched: 2026-06-03
- caveat: Fix: `loadDiff`/`scoreAgainstDiff` should EXCLUDE the spec file under review (`docs/specs/<slug>.md`) and arguably the whole `docs/specs/` + `docs/archive/` paths from the diff it scores against, so AC ids only count when they appear in IMPLEMENTATION/test added-lines, not in the spec prose. Without this, drift-check is a no-op-in-the-other-direction (always passes) during any workflow where the spec is still uncommitted in the tree. Pairs with the existing d1f7 secondary limitation (regression/meta ACs are never diff-line-traceable). Cross-ref: `.claude/skills/tdd/drift_check.mjs` `loadDiff` (working-tree diff), `scoreAgainstDiff`. Consider a `--exclude <glob>` arg or a hardcoded spec-path skip.
