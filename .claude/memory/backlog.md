...
## harness-auto-resume-after-consent-gate-2ddf
...
- verified-at: HEAD
- last-touched: 2026-05-17
- caveat: When `.claude/state/.harness_active` marker is present (set by the harness skill on `continue`), the post-consent-gate UX shouldn't tell the user to "run `/harness` again". The Stop hook (`harness_continuation`) is supposed to re-fire harness automatically when the loop exited at a gate; verify the path: marker present + gate token written by consent slash command → harness should auto-resume without a user prompt. If the prompt is coming from a phase-skill terminal message rather than the harness loop itself, that's where the fix lands.
