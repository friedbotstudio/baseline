...
- Role: exports `writeConstraint(memDir, key, fields)` (the **only** sanctioned way to create a constraint — `/memory-sync` Step 4.5 calls it rather than writing the file directly), `decisionsRestingOn(memDir, constraintKey)`, and `UnregisteredCategoryError`.
