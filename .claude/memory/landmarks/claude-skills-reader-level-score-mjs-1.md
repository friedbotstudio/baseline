...
- Companions: `.claude/skills/reader-level/SKILL.md` (the rewriting moves that actually shift the four measures), `.claude/skills/technical-writing/measure.mjs`.
- **Nothing tests this module.** The companion list named `tests/reader-level-score.test.mjs`, which does not exist and no rename accounts for; the only test naming `reader-level` at all is `tests/document-routing-gate.test.mjs`, and it asserts the delegate ORDER in the prose pipeline, never the scorer. Corrected 2026-08-26. Treat the scoring thresholds here as unpinned: a change to them fails no suite.
