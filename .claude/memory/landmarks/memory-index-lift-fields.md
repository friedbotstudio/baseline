...
- Consumers: `migrate.mjs` (+`reliftShards`), `shape.mjs`, `build-index.mjs`, `scoped-memory.mjs`, `standup/gather.mjs`, `memory-sync/next-q-id.mjs`, `memory-sync/sweep.mjs` (via `strandedFieldBullets`).
