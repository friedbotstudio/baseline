...
- Exports `isStaleFromFields`, `needsChangedSet`, `usableStamp`, `governsMatches`, `splitList`, and `STALE_DAYS`. The private `witness()` is the actual rule; the two public faces differ only in what they do with an unanswerable question.
- **Two readers, one predicate.** `memory-sync/sweep.mjs` and `hooks/lib/memory_session_start.mjs` each keep their own frontmatter parsing, then pass the fields in. The split is at the IO boundary, not the logic, so the rule stays pure and testable without git.
- **They no longer keep their own git call.** Both resolve the changed-set through [[claude-hooks-lib-memory-changed-set-mjs]] as of `session-start-stale-cache` (2026-08-26); the duplicated `changedSince` in each file is gone. `needsChangedSet` is the gate that decides whether to ask at all, and it mirrors this module's own early returns — `test_when_needs_changed_set_is_false_then_verdict_matches_a_computed_changed_set` is what holds the two statements of that condition equal.
