...
- `corpusDir()` lives here and validates `--spec-dir` with `assertNoTraversal` when relative, accepting absolute paths as-is. `memory-flush` and `system-reconcile` originally accepted the same flag with NO validation while this one checked it; that asymmetry was closed at `/simplify` 2026-08-08 and is the reason all three now import the same guard.
