...
- One EXPECTED_* set is **not** a literal: `DEFAULT_MCP_SERVERS` is derived from `readDocsProvider()` so the documentation-fetch server is named in one pointer rather than written into the audit. `checks/context.mjs` passes the audited tree's `rootDir`, so a consumer-tree audit expects that tree's provider. See [[claude-skills-lib-docs-provider-mjs]].
