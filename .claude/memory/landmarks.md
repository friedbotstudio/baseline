...
- Caveat: scope is deliberately narrow — fires ONLY on `.claude/state/epic/<slug>.json` writes, gates ONLY the false→true `approved` transition. Children[] appends, status flips, and idempotent re-writes of an already-approved epic pass through ungated (the `currentApproved` short-circuit at line 68). Existence + slug match only, NO TTL (an approved spec stays approved). The Bash/heredoc write surface is NOT covered — backlog key `extend-epic-approved-enforcement-to-bash-write-surface-abad` tracks closing that gap.
