...
- Companion: stage written by `src/cli/upgrade-tiers.js → writeStage` / `writeStageBaseless`; stage manifest read by `findPendingStage` (idempotency check) and by this skill. CLI's pointer emitted by `src/cli/tui/upgrade.js → run` and `bin/cli.js → dispatchUpgrade` when findPendingStage returns non-null. `.claude/hooks/memory_session_start.sh:1` scans `.claude/state/upgrade/*/manifest.json` for `status: PENDING` and emits a one-line nag. **The shipped marker helper at `.claude/skills/upgrade-project/marker.mjs:1` is the post-RECONCILED writer (replaced the v0.8.1 `node -e "import('./src/cli/reconciliation-marker.js')..."` invocation that broke in consumer installs because `src/cli/` doesn't ship); `src/cli/merge.js → threeWayMerge` remains the next-upgrade consumer via `src/cli/reconciliation-marker.js`.**
- Verified-at: 67da6dc
- Last-touched: 2026-05-27
