...
- Decision: S4 registered ONLY `sprint-channel` in `.mcp.json` (+ shipped `src/.mcp.template.json`) as `{command:"node", args:[".claude/mcp/sprint-channel/server.mjs"]}` — the 4th baseline MCP server. This is a **3→4** MCP-count cascade, NOT the 3→5 the roadmap prose originally implied.
