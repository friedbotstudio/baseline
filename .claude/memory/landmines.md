...
- caveat: re-confirmed live 2026-06-08 in the standup-skill workflow — adding the `standup` SKILL.md + editing `gather.mjs` at /simplify both drove `hash mismatch at .claude/skills/standup/gather.mjs` until `bash scripts/build-template.sh` restamped the manifest. Corrected the stale `.sh` path refs (the hooks + audit were ported to `.mjs`). See companion landmine [[baseline-skill-count-cascade]].
