...
- **The live instance is closed; the question is not.** `2542786` landed both Epic 11 rows (D and E now read SUPERSEDED) and `docs/roadmap-execution-plan.md` is clean. Do not go looking for the dirty file. What remains open is the policy: nothing mechanical still protects an orphaned edit when `/triage` replaces a `workflow.json`. Checked 2026-08-26.
