...
- Intent: the shipped power track declares `preconditions: [{name: requires_git}]` only. The SOP describes a `requires_config_flag` gate on `velocity.power_mode.enabled`, and the live file has it, so on a consumer install the power track is selectable before the flag is flipped. Confirmed at 3eafe4f.
