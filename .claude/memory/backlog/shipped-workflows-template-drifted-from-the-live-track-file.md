---
key: shipped-workflows-template-drifted-from-the-live-track-file
category: backlog
scope: []
status: picked-up
...
governs: src/.claude/workflows.template.jsonl, .claude/workflows.jsonl
...
- Intent: root cause behind two of the reported drift items. `.claude/workflows.jsonl` carries the power track's `requires_config_flag` fence and the chore track's `internal_phases[]`; `src/.claude/workflows.template.jsonl` carries neither, and no test asserts parity on those two fields. Fix the template and add the parity assertion in the same change, or the next edit re-opens it.
