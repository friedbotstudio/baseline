---
key: integrate-prereq-rejects-an-excepted-simplify
category: backlog
scope: []
status: open
...
governs: .claude/skills/integrate/SKILL.md
---
...
- Intent: `integrate`'s prereq reads "`simplify` in `completed` AND (`security` in `completed` OR `security` in `exceptions`)". Security gets an exceptions branch and simplify does not, so a chore whose right-size gate auto-skipped simplify can never satisfy it. Give simplify the same OR-exceptions branch security already has.
