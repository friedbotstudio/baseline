---
key: shipped-sensitive-globs-never-covered-hooks-or-commands
category: backlog
scope: []
status: open
...
governs: src/project.template.json, .claude/skills/chore/sensitive-surface.mjs
---
...
- Intent: MEDIUM. The shipped `security.sensitive_globs` is `[**/auth/**, **/*.env*, **/secrets/**, **/*credential*, **/security/**]` — it never contained `.claude/hooks/**`, `.claude/commands/**`, `src/cli/**` or `bin/**`. This is WORSE than reported: the entries were not dropped by `/init-project` tailoring, they were never in the shipped default, so every consumer install has the hole. A diff rewriting the commit guard reports not sensitive, and the chore track's security trigger cannot fire for the class of change it exists to catch.
