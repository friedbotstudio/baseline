...
- **Re-checked 2026-08-24.** `.claude/skills/memory-index/SKILL.md` does not exist and never has (`git log --all --follow` on that path returns nothing). The directory is a helper library (`categories.mjs`, `cli.mjs`, etc.) that `memory-sync/SKILL.md` imports from — not a standalone skill with its own frontmatter. The entry's original premise ("declares no `owner: baseline`") assumed a `SKILL.md` to tag; there is none. Article XII's hash-verification mechanism keys off `SKILL.md` frontmatter and has no path to a bare helper directory. The exposure this entry flags — `categories.mjs` is an unhashed shared oracle for four downstream readers — is still real; the fix just cannot be "add `owner: baseline`" as originally proposed. Needs a human decision on how (or whether) the manifest hash should reach a helper directory with no `SKILL.md`. Flagged for the operator, not resolved.
