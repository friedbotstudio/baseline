---
key: no-track-declares-a-review-node
category: backlog
scope: []
status: open
...
governs: .claude/workflows.jsonl, .claude/hooks/track_guard.mjs
---
...
- Intent: zero tracks declare a node with `metadata.phase == 'review'`, yet `track_guard` treats `review` specially (`track_guard.mjs:143`) and `deriveExceptions` can never except a phase no track declares. The phase is structurally unreachable. Either drop the guard's special case or give the tracks that need it a review node.
