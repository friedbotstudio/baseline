...
// completion record for every phase newly present in completed[]. For any other
// path it is a no-op. It never blocks a tool call and never touches the edited
// file — PostToolUse cannot deny, and this hook writes only to the timing JSONL.
//
...
try {
  stampFromWorkflow({ rootDir: CLAUDE_PROJECT_ROOT });
} catch {
