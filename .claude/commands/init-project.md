...
2. **`project.json` proposed values** (from JSON `project_json`, merged with baseline defaults):
   ...
   - `security.sensitive_globs` — the template ships generic cross-stack defaults (`**/auth/**`, `**/*.env*`, `**/secrets/**`, `**/*credential*`, `**/security/**`). From the Step 3 survey, propose the **project's actual** sensitive surfaces (the real auth module path, secret/credential directories, key material, payment or PII handling, IaC and deploy credentials) and confirm the merged list via `AskUserQuestion`. Defaults are a floor, not a ceiling: propose additions, never silently drop a shipped glob. These globs drive the right-size gate's `sensitive_surface_unreviewed` advisory and each `epic-child`'s `security` risk escalation, so an empty or wrong list means a real security surface changes without anyone being told.
