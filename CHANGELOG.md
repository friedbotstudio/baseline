...
- `plantuml_syntax_guard` no longer spawns a JVM during spec authoring. It is **advisory by default** — it notes that PlantUML fences in a written spec are unvalidated at the write boundary and points to `/spec-lint` — and runs the strict `java -jar … -checkonly` parse only when `project.json → plantuml.strict_syntax_check` is `true` (default `false`, in both this repo and the shipped consumer `project.template.json`). Authoritative syntax validation stays on-demand in `/spec-lint`, before `/approve-spec`. Amended `docs/init/seed.md` §4.1 and `CLAUDE.md` Article VIII accordingly (with their `src/` byte-mirrors), and updated the public `site-src/hooks.njk` row.
