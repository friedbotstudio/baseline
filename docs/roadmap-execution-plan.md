...
- ⬜ T11. Re-home the 87 landmarks still carrying the category default `scope: [scout]`, which T8 deferred. T8 curated the other two cohorts — the 47 entries stamped with the retired placeholder and the 49 landmines at the five-phase default — and moved the spec write from 107 surfaced facts to 61 and security from 62 to 22. The landmarks were left because the obvious fix does not work: the path leg fires on WRITING a governed file, and scout writes only `docs/scout/<slug>.md`, which no landmark governs, so re-homing would remove landmark surfacing from scout entirely rather than narrow it. Scout therefore still surfaces 96. What this needs is the third mechanism T8's spec named as a non-goal — a relevance filter over the workflow's declared write surface — so a scout write surfaces the landmarks for the paths that workflow will touch. Spec AC-009 tagged the deferral `deferred: risk` and a test locks the count at exactly 87, so this cannot drift closed unnoticed.
