...
- **`node` ≥ 18.17 on PATH.** Every hook AND every skill helper runs as a Node ESM script (`.mjs`); no `bash` or `jq` at hook runtime. `npx` is also required for the `plantuml` and `playwright` MCP servers (`baseline` runs on `node`; the shipped documentation provider is a remote HTTP server and needs neither).
...
### §2.5 Current-docs rule (the provider is a default, not a mandate)

When writing or reviewing code that uses any third-party library, always verify its API against current documentation rather than training-data recall. This is a **capability requirement, not a tool mandate** — it is satisfied by any current-docs source: the documentation provider the project declares, a library's official docs or `llms.txt`, or a pinned local doc cache. Never assume an API from training recall.

**Which server plays that role is a pointer, not a constant.** `.claude/docs-provider.json` holds one field, `provider`, naming the MCP server that acts as the documentation fetcher; `.claude/skills/lib/docs-provider.mjs` resolves it and falls back to the shipped default when the pointer is absent or unreadable, so a broken pointer can never stop a skill from checking an API. Governance cites the pointer and never the vendor — that is what makes a swap a config edit rather than an amendment. Changing provider is two edits: replace the server entry in `.mcp.json`, then name it in the pointer. Self-hosting the shipped provider is one edit: change its `url` and leave the pointer alone. The procedure lives with the commands a consumer runs (`/init-project`, `/upgrade-project`), not here.

The baseline ships a default satisfier in `.mcp.json` so the capability travels with the project; a project MAY replace or remove it provided the verify-against-current-docs outcome still holds. Rationale: the baseline is open-source and SHALL NOT hard-couple downstream users to any one commercial service (login / paid tier) — U6, no irreplaceable dependency.
...
```
<repo-root>/
├── .mcp.json                   # project-level MCP servers (gitmcp, plantuml, playwright, baseline)
...
│   ├── hooks/                  # 27 hook scripts: 22 write/run-boundary guards + 4 lifecycle hooks + 1 input-boundary hook (Node ESM, no jq)
...
│   ├── skills/                 # 59 skills: artifact (4) + phases (10) + workers (5) + spec helpers (5) + orchestration (3) + memory (1) + navigation (1) + phase helpers (1) + generators (4) + shared globals (10) + audit (1) + alt tracks (2) + maintenance (4) + sprint (5) + roadmap (3)
│   ├── output-styles/          # 1 output style: Baseline (§4.9) — enabled by settings.json → outputStyle
│   ├── memory/                 # project memory: 8 canonical categories (flat <name>.md OR sharded <category>/<key>.md when memory.sharded_store activated) + _pending.md + _resume.md + _thread.md (all gitignored body) + README.md
...
│   └── memory/<8 canonical>.template.md
└── docs/
...
### §4.1 Hooks (27 total — 22 write/run-boundary guards + 4 lifecycle hooks + 1 input-boundary hook)
...
| `state_write_guard` | PreToolUse / Bash + Write\|Edit\|MultiEdit\|NotebookEdit | Denies a **subagent** write to `.claude/state/**` on BOTH tool boundaries — a shell redirect appends a phase to `completed` as well as an Edit does, so wiring only the editing tools left the privilege path open (found at `/security` in the introducing cycle). The Bash leg is target-anchored and reuses the consent detector's machinery via `writesPathFamily` (variable expansion, executed-fragment scan), so a READ passes: a subagent that cannot read `workflow.json` cannot work. Article II puts decisions in main context, and workflow state is where a decision becomes durable — a subagent appending a phase to `completed` widens what `track_guard` authorizes next. A subagent is identified by a non-empty `agent_id` on the payload; the harness sends `agent_id`/`agent_type` only inside a subagent, so an absent key reads as the main session. `swarm-worker` is denied here too: Article II sanctions it to write CODE in its worktree, not workflow state, and the guard scopes to `.claude/state/**` and nothing wider so its source writes keep working. Fail-open on a main-session write, a path outside the state directory, a degenerate payload, or any read error. Pure `decideStateWrite()` lives in `hooks/lib/state-write.mjs` and is unit-tested without executing the hook (`tests/state-write-guard.test.mjs`). |
...
| `process_lifecycle_guard` | PreToolUse / Bash + Write\|Edit\|MultiEdit\|NotebookEdit | Advisory-only; emits `info` decisions and never blocks. **Bash leg**: detects process-management patterns (`kill`, `pkill`, `lsof`, `fuser`, `npm run.*(serve|dev)`, `eleventy --serve`, `vite`, `next dev`, `astro dev`, `python.*http.server`) and surfaces the matching entries by category and key (`landmines/lsof-port-kill-takes-firefox-with-it`, `conventions/dev-server-ownership`) through `resolveCategory`, so it reads the flat and sharded store shapes alike. Closes the gap where ad-hoc main-context Bash had no skill-driven memory pull. **Write leg**: three surfacing triggers, composed before the allow so none can suppress another — **phase-scoped** (a write under `docs/{specs,intake,scout,research,security}/` surfaces every fact whose `scope:` names that phase), **path-governed** (any other path surfaces the entries whose `surfaces-on:` globs match it, falling back to `governs:` when an entry declares no separate audience), and **corpus-location** (the same path walks up through `docs/system/` to the element that anchors it and the concepts that own it, §4.8). The payload's absolute `file_path` is normalised to a repo-relative path first — both lookups match relative anchors, so without it neither fires. |
...
| `phase_timer` | PostToolUse / Write\|Edit\|MultiEdit + Bash | Observation-only. When `.claude/state/workflow.json` is written — by the edit matcher or any Bash command that mutates it (redirect, node-fs, jq) — stamps a `{phase, event:"completed", ts, wait_ms, batch_id, batch_size}` record into `.claude/state/timing/<slug>.jsonl` for every phase newly present in `completed[]` (`wait_ms` carries consent-gate wait, zero on work phases; `batch_id`/`batch_size` mark rows emitted by one stamping call, so a batched row reads as *observed together* rather than *cost nothing*; the `run-start` baseline row anchors at `created_at * 1000` so the JSONL stays readable after `/commit` archives `workflow.json`) (idempotent; never blocks; never mutates the edited file). The Bash leg attempts the stamp unconditionally; the idempotent early-return makes it cheap. It also sub-stamps `{phase:"tdd:<tick>", event:"sub"}` rows from `workflow.json → tdd_ticks[]` (gated by `artifacts.subtick_timing.enabled`, default on), so `timing.md` nests a per-worker-tick breakdown (scenario/implement/verify/…) under the `tdd` rollup. `lib/timing.mjs` holds the pure stamp + render logic; `/archive` renders a per-phase model-vs-human-wait `timing.md` into the bundle. Velocity Lever 0 — measures where a workflow run's wall-clock goes. |
| `memory_session_start` | SessionStart | Scans `.claude/memory/*.md`, prints a compact index (per-file entry counts, stale counts, pending-candidate count) into Claude's startup context, and appends `_resume.md` (the cross-session continuity snapshot) with a source-aware framing line (`compact` / `clear` / `resume` / `startup`). Total `additionalContext` capped at ~9.5KB. |
| `memory_stop` | Stop | At end of every assistant turn, reads the transcript, extracts memory candidates (touched source paths → landmarks; documentation-provider queries → libraries; user/assistant text-block intent lines → backlog), appends to `.claude/memory/_pending.md`, and refreshes `.claude/memory/_resume.md` for next session. Passive collector — never writes to canonical memory files. |
...
**Template-rendered at build time; rewritten in place thereafter.** The worker's canonical body lives at `src/agents/swarm-worker.template.md`, a **development-tree** artifact. The file at `.claude/agents/swarm-worker.md` is its rendered output and is what ships. The template carries four tokens — `{{NAME}}`, `{{DESCRIPTION}}`, `{{SKILLS}}`, `{{ROLE_LINE}}`.

`/init-project` does **not** read that template: a consumer install receives no `src/`, so a re-render step pointed at it can never execute there. Instead `/init-project` rewrites the `skills:` block of the installed `.claude/agents/swarm-worker.md` in place, which is byte-identical to re-substituting `{{SKILLS}}` because every other token is already resolved in the shipped file. Render parity between template and rendered output is asserted by test, not assumed — it had silently drifted twice before the assertion existed.

**Automated re-rendering by `/init-project`.** Step 6.4 rewrites the `skills:` block of the installed `swarm-worker.md` in place, driven by the recommender's `additions.swarm_worker_skills`. The recommender does **not** propose new subagent types — only stack-skill additions for the existing worker. Specialization happens via skills loaded into the worker's context, not via parallel agent personas; new decision-making roles belong in skills, which run in main context.
...
- `research` — Phase 3. Surfaces 2–4 candidate solution approaches with tradeoffs, grounded in current library docs via the project's documentation provider. Output: `docs/research/<slug>.md`.
...
- `document` — Phase 10. Orchestrator. Routes any page on a documentation surface to `technical-writer` (which runs `technical-writing` → `reader-level` → `humanizer` and gates on both scorers), delegates standalone technical reference to `documentation`, tutorials to `technical-tutorials`, and **all other prose** to the `prose` skill (which applies `humanizer` mandatorily).
...
- `roadmap-sync` — Phase 10.6. Flips the roadmap tasks named in `workflow.json → roadmap_tasks[]` (⬜→✅) and promotes their epic headings in `project.json → roadmap.path`, preserving the standup parser format contract. Fail-open: no-ops when the path is unset/absent or no task matches; never blocks a commit. Runs on every committing track. On the `epic` track it appends instead of flipping — the epic's heading plus one row per slice, tagged with the epic slug and stamped back as `roadmap_epic` — so an `epic-child` can name its own row.
- `memory-sync` — Phase 10.7. Curates `_pending.md` candidates with full workflow context (or fast-paths on empty pending while still running canonical Step 0 sweeps). Canonical memory writes ship in the same commit as the work that motivated them.
...
- `implement` — writes production code that turns failing tests green inside an explicit `write_set` (mandatorily `code-structure`; current docs for any third-party API). Used by `/tdd` Step 3 and by `swarm-worker` Step 3.
...
**Shared globals (10)** — skills the baseline *uses* heavily; vendored into `.claude/skills/` so they travel with the repo and have no external runtime dependency:
...
- `reader-level` — written for this baseline (Friedbot Studio); the repo license applies. Brings documentation prose to a target reading grade without losing precision. Ships `score.mjs`, a four-measure scorer (grade, hard words, clause load, jargon load). Invoked by `technical-writer` between the draft and `humanizer`.
- `technical-writing` — written for this baseline (Friedbot Studio); the repo license applies. Carries the craft rules and per-Diátaxis-type numeric targets derived from a 113,887-word corpus of documentation published before 2022. Ships `measure.mjs` + `corpus-bands.json`, a 24-axis scorer with a calibrated pass threshold.
- `technical-writer` — written for this baseline (Friedbot Studio); the repo license applies. The documentation-page SOP: prepare context → `technical-writing` → `reader-level` → `humanizer`, then both gates. Delegate target from `/document` for any page on a docs surface.
...
**Memory (1)**:
- `memory-sync` — Phase 10.7. Curate the auto-extracted candidates in `_pending.md` into the canonical files, then reset the pending body. Rejects any promotion that lacks a required `verbatim:` (Art. IX.6).
...
- `system-reconcile` — report the standing corpus's health across seven checks (gaps, stale, dangling, duplicate anchors, orphan shards, unillustrated, missing kind annotation) and repair only what a human confirms. Exports no writer reachable from a workflow phase, which is what keeps `/archive` the corpus's single writer.

**Roadmap (3)**:
- `roadmap` — read-only view of the plan: epics with tallies, open rows nested beneath their epic, finished epics folded into one rollup line, and the next planned task in file order. `--all` expands, `--epic N` scopes. Verbs `list` / `tasks` / `epics` / `next`. Never writes; not a workflow phase.
...
- `roadmap-sync` — Phase 10.6. Flip landed `roadmap_tasks[]` from ⬜ to ✅ and promote their epic headings; on the `epic` track, append the epic's heading and slice rows instead. Fail-open; every committing track.
...
- **`gitmcp`** — a remote streamable-HTTP server at `https://gitmcp.io/docs` (Apache-2.0, no auth, self-hostable). Live library documentation lookup: it resolves a library name to an owner/repo and fetches or searches that repository's documentation. The **default satisfier** for §2.5 and the name `.claude/docs-provider.json` ships with — replaceable at will, since §2.5 mandates the current-docs outcome rather than any one tool. Self-hosting changes only the `url`; switching provider changes this entry and the pointer.
...
- **`baseline`** — `node .claude/mcp/baseline/server.mjs`. The one **first-party** server: a file-locked, race-safe coordination channel exposing eighteen tools (`register_peer`, `send_message`, `broadcast`, `claim_task`, `signal_done`, `raise_conflict`, `yield_fork`, `release_task`, `leave_peer`, the Article X escalation surface `ask_lead`, `answer_peer`, `sprint_status`, `enqueue_task`, the general task-management surface `update_task`, `cancel_task`, `list_tasks`, which works against a per-repository `default` channel when no `sprint_id` is given, plus `acquire_lead` and `release_lead`, which hold one lead per channel) over `.claude/state/sprint/<sprint_id>/`. Backs the org-team model (Article X) and sprint-mode dogfooding. Unlike the other three it is **not** fetched by `npx` — `scripts/bundle-mcp-servers.mjs` (build Stage 1.7) inlines `@modelcontextprotocol/sdk` and `zod` with esbuild so the shipped artifact is self-contained and the baseline stays zero-runtime-dep. It is the **only** first-party server: the `sprint-pool` channel server and the `sprint-broker` socket transport it spoke through were retired in 0.26.0, because the push they provided needed a launch flag no consumer install can assume, while everything they coordinated already reconciles through `sprint_status`. Their surviving duty — one lead per channel — is now `acquire_lead` / `release_lead` against the store, and pushed notification is an accelerator that degrades to reconcile-only when the host cannot carry it.
...
**Default-on, with no installer surface.** There is no `--no-output-style` flag: unlike §15.5's CI posture, which writes outside `.claude/` and therefore needs one, this is a single key inside `.claude/` whose opt-out already exists — a consumer's own `settings.local.json` overrides it, or they delete the line. An upgrading consumer who edited `settings.json` is staged for `/upgrade-project` rather than auto-merged, because that file carries all 27 hooks' wiring.
...
**Outcome-led argument (CLAUDE.md XI.1).** The register stays constitutional — noun-heavy, declarative, mechanism-naming. The *argument* is outcome-led. On user-facing copy a section headline SHALL assert what becomes true for the reader, not name a topic; the mechanism follows the claim and never replaces it. "A blocked tool call never runs" is a headline; "Why hooks" is a table of contents. This scopes how the case is made, not what may be said: every anti-reference in `PRODUCT.md` (no gradient text, no hero-metric vanity, no "AI-powered", no hyperbole, no mascots) remains binding, and the verifiability rule is unchanged — a claim may only assert what the codebase can be checked against.
...
- **Pillar 4: tying open ends** — `/simplify` → `/security` (unless in exceptions) → `/integrate` → `/document` → `/archive` → `/roadmap-sync` → `/memory-sync` → **yield at `/grant-commit`** → `/commit`.
...
**Branch-aware consent policy.** `git_commit_guard` reads the current branch via `lib/common.mjs → currentBranch()` — `git symbolic-ref --short HEAD` — on every `git commit` / `git push` invocation. An **unborn** branch (a repository initialized with no commit yet) resolves like any other, so a repository's first commit is governed by the same policy as its thousandth. Inside a work tree the branch always resolves; outside one it resolves to nothing and Article VII is vacuous. The guard routes per:
...
On a **protected branch**, commit requires fresh `commit_consent` (`/grant-commit`), push requires fresh `push_consent` (`/grant-push`). On a non-protected branch, both proceed without consent. **Detached HEAD** (`git symbolic-ref` fails inside a work tree, so `currentBranch()` returns the literal `HEAD`) denies both — branch-aware policy needs a named branch.
...
- `swarm.max_parallel`, `swarm.isolation: "auto"`, `swarm.min_tasks_worth_swarming: 1`, `swarm.refuse_dirty_tree: true`, `swarm.exempt_path_prefixes`, `swarm.enforced_path_prefixes`.
...
- **Warm-context headroom target (2026-08-10, `warm-context-diet`).** `CLAUDE.md` is loaded in full at every session start, so its size is charged against the context budget before the user types. Beyond the 40,000-char hard cap, the governance suite pins an advisory target of **28,000 characters** (`tests/warm-context-diet.test.mjs`). The relocation that established it moved, under the clause above: the per-hook behavior table (Art. VIII) to annex §2; the gate-handshake narration, per-track paragraphs and swarm-vs-solo rationale (Art. IV) to annex §5.13; the elaborative bodies of XI.1–XI.5 and XI.12 to annex §5; the Article X detail to annex §5.6; the integrate-failure decision tree (Art. V) to `.claude/skills/harness/SKILL.md`, which that Article already names as the SOP's home; and the project-agnostic greeting string (Art. III) to annex §2. Every relocated block left its binding clause and an annex pointer in place. **Article VI moved nothing** — the non-negotiable engineering rules are the rules most often violated when unseen, so they earn their warm cost; a test pins their sha256.
...
- **Git hooks are not Claude Code hooks.** The posture adds no entry to `.claude/hooks/`; the roster stays 27.
...
`workflows.jsonl` supersedes the hardcoded triage templates (intake-full / spec-entry / tdd-quickfix / chore). Triage reads `workflows.jsonl` at seed time, validates each Track, classifies the user's request, and materializes the chosen Track's DAG into the TaskList. Three of the canonical four tracks shipped in the pristine template — `intake-full`, `tdd-quickfix`, `chore` — remain byte-equivalent to the pre-§18 hardcoded templates per spec AC-016 (`tests/byte-equivalent-migration.test.mjs`). **`spec-entry` is a sanctioned exception to that equivalence**: it declares an `intake` node followed immediately by gate A, then a `scout` node ahead of `spec`, so its golden fixture is pinned to the post-amendment DAG rather than to the pre-§18 template. Both halves are evidence-driven. Joining the archived `workflow.json` bundles to the phase-timing logs, `spec-entry` was simultaneously the most-used track and the slowest — slower than the fuller `intake-full` above it — while shipping a smaller median diff, with the excess concentrated in the post-approval implementation span rather than in spec authoring or human wait. The structural difference between the two tracks was that `spec-entry` drafted its spec with no codebase map, so its design guesses were discovered during implementation and paid for as rework. Scout is the cheapest phase that removes that class of guess. The `intake` node repairs a second defect, in the gate rather than the discovery. `/approve-direction` approves the DIRECTION and records a whole-file content hash of the artifact it is handed; §18's gate-collapse (D3/CO-E) makes the spec machine-reviewed and explicitly not human-gated. But `spec-entry` declared no intake, so the command had no direction document to resolve and the token came to hash the **spec** instead — measured across the archive, 110 of 113 approval tokens point at `docs/specs/` and only 3 at `docs/intake/`. A spec-bound token re-yields at gate A on any byte change, so a typo fix cost the same human round-trip as a scope change, which is the re-approval loop the gate-collapse was meant to end. Placing `intake` first and gate A immediately after it — the ordering `intake-full` already uses — means the token is written before a spec exists and can only ever hash the direction. The repair is structural: a severity carve-out that let a model call its own amendment minor would be self-approval, and is not sanctioned. `spec-entry` now skips only `research`. A fifth selectable track, `freeform`, is a §18-native addition with no pre-§18 byte-equivalent counterpart: its DAG carries only the closing sequence (`memory-sync` → `grant-commit` → `commit`) and relies on blanket exceptions across every pre-commit phase to silence track-ordering while keeping every hook active. The 11-track inventory: 9 selectable (intake-full, spec-entry, tdd-quickfix, chore, freeform, epic, epic-child, org, power) + 2 sub-tracks (swarm-implementation, tdd-worker-chain). The `power` batch-sprint track is opt-in via `velocity.power_mode.enabled` and requires git. The `epic` / `epic-child` pair (§18.9) amortizes feature-scoped discovery across the subtasks of one feature.
...
- **`epic`** (selectable) — discovery only. Canonical DAG: `intake → scout → research → spec → approve-direction → memory-sync → grant-commit → commit` (per-project review nodes such as `spec-shippability-review` insert before `approve-direction` exactly as they do in `intake-full`). It produces a **sliced spec** at `docs/specs/<epic>.md` (one `## Slice <id>` section per future child, each grouping the ACs that child owns — **slice-section grammar**, below), plus `docs/scout/<epic>.md` and `docs/research/<epic>.md`. It is approved **once** (one `/approve-direction` covers all slices) and commits the discovery bundle **live** — it deliberately omits `archive`, because the discovery is the deliverable of that commit and SHALL remain resolvable at its `docs/` path for children to pin. The epic stays *open* until its children resolve; the whole discovery bundle is then archived **automatically when the last child commits** — the `commit` skill's epic-child path (commit/SKILL.md Step 2.8) flips that child to `committed` pre-commit and invokes `.claude/skills/commit/epic_close.mjs <epic>`, which `git mv`s the bundle into `docs/archive/<date>/<epic>/` (riding that same commit, no separate consent gate) and merges `closed:true` + `closed_at` into the epic state file. Run outside the harness, `epic_close.mjs` is the idempotent standalone recovery path: it stages the move and prompts for a normal `/grant-commit` + `/commit`. It never writes `approved` and retains the state file. (Children still archive only their own slice artifacts separately.)

**Slice-section grammar (binding — the machine contract every reader parses).** The heading and the AC list of a `## Slice <id>` section are read by `.claude/hooks/lib/pinned-spec.mjs`, `.claude/skills/spec-lint/lint.mjs` and `.claude/skills/tdd/drift_check.mjs`, all through the single declaration at `.claude/skills/lib/slice-grammar.mjs`. A spec author who follows this paragraph passes every one of them:

- **Heading** — `## Slice <id>`, optionally followed by a title: `## Slice B1 — ports and the composition root`. The `<id>` is a word not followed by another word character or a hyphen, so `B1` never matches `## Slice B10`. The anchor is `slice-<id>`, which is the fragment an `epic-child` pins.
- **AC list** — one bold-labelled line inside the section, with or without a leading bullet, under either label: `- **ACs**: AC-001, AC-002` or `**Acceptance criteria**: AC-001, AC-002`. Only that line supplies the slice's ids; an `AC-NNN` mentioned anywhere else in the section body is prose, not a claim.
- **`slices[].acs` in the epic state file** holds `AC-NNN` **ids**, never criterion prose. `.claude/skills/lib/epic-acs.mjs` is the shared predicate, asserted at the write.

A reader that stops accepting any form above is a defect, not a stricter check: widening a reader is a fix, narrowing one breaks consumer installs silently. `.claude/skills/conformance/` holds the adversarial fixture that pins this, run by `npm test` and by `audit-baseline`. Run it directly with `node .claude/skills/conformance/cli.mjs` (add `--json` for the full result); it exits non-zero on a disagreeing reader or on a run that measured nothing.

- **`epic-child`** (selectable) — fast implementation path for one slice. Declared DAG: `tdd → simplify → security → integrate → document → archive → roadmap-sync → memory-sync → grant-commit → commit`, of which `simplify`, `security`, and `document` are written into `workflow.json → exceptions` **by default** (effective fast path `tdd → integrate → archive → roadmap-sync → memory-sync → grant-commit → commit`) and escalated into a given child — removed from `exceptions` — only when `/triage` risk-flags the slice (see *Conditional review weight* below). It does **not** re-run any discovery phase; it inherits the epic's. The full chain is declared (not a trimmed DAG) so escalation is a triage-time exception edit, never a TaskList reshape.
