...
- **`sprint-channel`** — `node .claude/mcp/sprint-channel/server.mjs`. The one **first-party** server: a file-locked, race-safe coordination channel exposing thirteen tools (`register_peer`, `send_message`, `broadcast`, `claim_task`, `signal_done`, `raise_conflict`, `yield_fork`, `release_task`, `leave_peer`, plus the Article X escalation surface `ask_lead`, `answer_peer`, `sprint_status`, `enqueue_task`) over `.claude/state/sprint/<sprint_id>/`. Backs the org-team model (Article X) and sprint-mode dogfooding. Unlike the other three it is **not** fetched by `npx` — `scripts/bundle-mcp-servers.mjs` (build Stage 1.7) inlines `@modelcontextprotocol/sdk` and `zod` with esbuild so the shipped artifact is self-contained and the baseline stays zero-runtime-dep. The sibling `sprint-pool` server is bundled too but deliberately **not registered** here. It IS a stdio server (the channel contract requires stdio); what keeps it unregistered is that Claude Code channels are in **research preview** — custom channels are off the approved allowlist and need `--dangerously-load-development-channels`, and Team/Enterprise organizations must explicitly enable channels before any load. None of that can be assumed on a consumer install, so the four Article X escalation tools live on `sprint-channel` instead and the shipped org path needs no flag. `sprint-pool` remains the opt-in push accelerator.
