...
- **`node` ≥ 18.17 on PATH.** Every hook AND every skill helper runs as a Node ESM script (`.mjs`); no `bash` or `jq` at hook runtime. `npx` is also required for the MCP servers (`context7`, `plantuml`, `playwright`; `baseline` runs on `node`).
...
```
<repo-root>/
├── .mcp.json                   # project-level MCP servers (context7, plantuml, playwright, baseline)
...
**Step 2:** Write `.mcp.json` with the four server declarations (`context7`, `plantuml`, `playwright`, `baseline`).
