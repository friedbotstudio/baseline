...
14. **(L1 — sprint-planner)** Given an already-decomposed body of work (a tasks file, approved epic slices, or open backlog entries), when `sprint-planner` runs, then it computes per-task readiness, orders with the ported generic graph engine when dependency edges exist (cycle/dangling/ordering proof) and degrades to declared status-only readiness when none do, and emits a proposal-only sprint manifest the human confirms before `/triage` — it never starts, stages, or commits work. (Maintainer scope increase 2026-07-05: "this is baseline product … we can make it a bit generic but it will work 100%".)
15. **(L2 — power track)** Given `velocity.power_mode.enabled: true` on a git project, when a batch of related spec-committed tickets is triaged to the `power` track, then the mechanical phases run once for the batch while `security` reviews each ticket individually (a per-ticket skip is forbidden), and the commit phase lands the batch as ordered Conventional Commits with the closing workflow state on the final commit; off-flag, behavior is byte-unchanged and the track is not selectable.
