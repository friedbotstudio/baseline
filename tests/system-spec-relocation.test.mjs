...
describe('A2 — the corpus is relocated to docs/system/', () => {
  it('test_when_corpus_relocated_then_docs_system_holds_every_record', () => {
    ...
    assert.equal(elements, 113, 'docs/system/elements must hold every element record');
    assert.equal(diagrams, 113, 'docs/system/diagrams must hold one shard per element');
  ...
  it('test_when_readers_repointed_then_readall_resolves_at_new_path', async () => {
    ...
    assert.equal(elements.length, 113, 'readAll must resolve the corpus at its new root');
