...
function anchoredSource(root, specDir, id, rel, lines) {
  const abs = join(root, rel);
  mkdirSync(join(abs, '..'), { recursive: true });
  writeFileSync(abs, lines.join('\n') + '\n', 'utf8');
  writeWorkspaceElement(specDir, id, { anchor: rel });
  return abs;
}

// The pinned contract is deriveEdges(rootDir, elements) — the element RECORDS,
// not a specDir. Reading them through the real store keeps the tests honest about
// the shape the implementation will actually receive.
async function elementsOf(specDir) {
  const store = await tryImport(STORE);
  assert.ok(store, `${STORE} does not exist yet`);
  return store.readAll(specDir).elements;
}
...
describe('B — edge derivation', () => {
  it('test_when_derivation_runs_on_seeded_corpus_then_import_edges_reproduce', async () => {
    ...
    anchoredSource(root, specDir, 'hooks-common-lib', 'lib/common.mjs', ['export const x = 1;']);
    anchoredSource(root, specDir, 'surfacing-triggers', 'hooks/trigger.mjs', [
      "import { x } from '../lib/common.mjs';",
      'export const y = x;',
    ]);

    const derived = edges.deriveEdges(root, await elementsOf(specDir));
  ...
  it('test_when_import_clause_spans_lines_then_edge_still_derived', async () => {
    ...
    const { root, specDir } = makeProject();
    makeWorkspace(specDir);
    anchoredSource(root, specDir, 'multiline-target', 'lib/common.mjs', [
      'export const readPayload = 1;',
      'export const projectGet = 2;',
    ]);
    anchoredSource(root, specDir, 'multiline-importer', 'hooks/guard.mjs', [
      'import {',
      '  readPayload,',
      '  projectGet,',
      "} from '../lib/common.mjs';",
  ...
  it('test_when_two_elements_share_state_path_then_state_edge_present', async () => {
    ...
    const { root, specDir } = makeProject();
    makeWorkspace(specDir);
    anchoredSource(root, specDir, 'writer', 'a/writer.mjs', ["const p = '.claude/state/workflow';", 'export default p;']);
    anchoredSource(root, specDir, 'reader', 'b/reader.mjs', ["const p = '.claude/state/workflow';", 'export default p;']);
  ...
  it('test_when_element_calls_project_get_then_config_edge_present', async () => {
    ...
    const { root, specDir } = makeProject();
    makeWorkspace(specDir);
    anchoredSource(root, specDir, 'branch-guard', 'hooks/branch.mjs', [
  ...
  it('test_when_skill_md_invokes_skill_then_orchestration_edge_present', async () => {
    ...
    const { root, specDir } = makeProject();
    makeWorkspace(specDir);
    anchoredSource(root, specDir, 'humanizer', 'skills/humanizer/SKILL.md', ['# humanizer', 'Prose pass.']);
    anchoredSource(root, specDir, 'prose', 'skills/prose/SKILL.md', [
      '# prose',
  ...
  it('test_when_edges_derived_then_every_edge_has_provenance_derived', async () => {
    ...
    const { root, specDir } = makeProject();
    makeWorkspace(specDir);
    anchoredSource(root, specDir, 'base', 'lib/base.mjs', ['export const b = 1;']);
    anchoredSource(root, specDir, 'user', 'app/user.mjs', [
