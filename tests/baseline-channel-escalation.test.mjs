// T3 — org-dispatch calls eight tools; baseline (the REGISTERED server)
...
//
// D-1 puts them on baseline so a consumer runs org mode with no flag.
//
...
const HANDLERS = path.join(REPO_ROOT, '.claude/mcp/baseline/handlers.mjs');
const SERVER = path.join(REPO_ROOT, '.claude/mcp/baseline/server.mjs');
...
describe('T3 — baseline carries the escalation surface', () => {
  ...
  it('test_when_existing_four_tools_then_names_and_shapes_unchanged', () => { // regression
    ...
    for (const added of ['ask_lead', 'answer_peer', 'sprint_status', 'enqueue_task']) {
      assert.ok(server.TOOL_NAMES.includes(added), `baseline must now expose "${added}"`);
    }
  });
});
